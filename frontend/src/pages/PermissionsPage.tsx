...
function FeatureMatrix() {
  ...
  const mut = useMutation<any, any, { feature_key: string; role: string; can_view: boolean; can_edit: boolean }>({
...
// ── General settings — currently just the app name ──────────────────────────
function GeneralSettings() {
  const qc = useQueryClient()
  const { data, isLoading } = useQuery({ queryKey: ['app-settings'], queryFn: getSettings })
  const [value, setValue] = useState('')

  useEffect(() => { if (data?.app_name) setValue(data.app_name) }, [data?.app_name])

  const mut = useMutation({
    mutationFn: () => updateSettings(value),
    onSuccess: () => {
      toast.success('Saved')
...
export function PermissionsPage() {
  const [tab, setTab] = useState<'matrix' | 'roles' | 'invite' | 'settings'>('matrix')
    ...
    <>
      ...
      <div className="p-6 space-y-5">
        <div className="flex gap-0.5 bg-gray-100 p-0.5 rounded-lg w-fit">
          {(['matrix', 'roles', 'invite', 'settings'] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              ...
              {t === 'matrix' ? 'Feature Permissions' : t === 'roles' ? 'Role Assignment' : t === 'invite' ? 'Invitations' : 'General Settings'}
            </button>
          ))}
        </div>
        {tab === 'matrix' ? <FeatureMatrix/> : tab === 'roles' ? <RoleAssignment/> : tab === 'invite' ? <InvitationsPanel/> : <GeneralSettings/>}
      </div>
    </>
  )
}
